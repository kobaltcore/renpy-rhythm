...
def load(fileobj):
    ...
    if not hasattr(fileobj, "readline"):
        raise TypeError(
            "fileobj must be a file object " '(or something with a "readline" method)'
        )
    instruments = []
    chart = None
    line = True
    ...
    if raw_name == flags.EVENTS:
        ...
        while line != "}":
            ...
            if line in "{}":
                continue
            match = re.search(
                r"([0-9]+)\s*=\s*" + flags.EVENT.value + r'\s+("?.*"?)', line
            )
            time = int(match.group(1))
...
def _parse_inst(fileobj, first_line):
    ...
    difficulty, kind = re.match("([A-Z][a-z]+)([A-Z][A-Za-z]+)", raw_name).groups()
    inst = Instrument(
        kind=flags.Instruments(kind), difficulty=flags.Difficulties(difficulty)
    )
    line = ""
    while line != "}":
        line = fileobj.readline().strip()
        ...
        if match is not None:
            ...
            time, kind, evt = re.search(
                r'([0-9]+)\s*=\s*(E)\s+"?([a-zA-Z]+)"?', line
            ).groups()
