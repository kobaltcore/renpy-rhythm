...
class Instrument(list):
    ...
    def __init__(self, kind=None, difficulty=None, notes=None):
        ...
        if kind is not None:
            if not isinstance(kind, flags.Instruments):
                raise TypeError(
                    "Expected an instrument ennum, got {}".format(type(kind).__name__)
                )
            self.kind = kind
        if difficulty is not None:
            if not isinstance(difficulty, flags.Difficulties):
                raise TypeError(
                    "Expected a difficulty enum, got {}".format(
    ...
    def __repr__(self):
        ...
        return "<Instrument, first notes: {}>".format(repr(first_notes))
    ...
    def _check_note(note, kind=()):
        ...
        if kind != () and note.kind not in kind:
            raise TypeError(
                "Expected Note of type {} but got {}".format(
