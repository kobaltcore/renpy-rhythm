...
class _BaseNote:
    ...
    def __cmp__(self, other):
        if not isinstance(other, _BaseNote):
            raise TypeError("Cannot compare Note with {.__name__}".format(type(other)))
        return -1 if self.time < other.time else (1 if self.time > other.time else 0)
...
class Note(_BaseNote):
    ...
    def __init__(self, time, kind=_flags.NOTE, fret=0, length=0, flags=None):
        self.time = time
        self.kind = kind
        self.fret = fret
        self.length = length
...
class SyncEvent(_BaseNote):
    ...
    def __repr__(self):
        return "<SyncEvent {} = {} {}>".format(self.time, self.kind.value, self.value)
